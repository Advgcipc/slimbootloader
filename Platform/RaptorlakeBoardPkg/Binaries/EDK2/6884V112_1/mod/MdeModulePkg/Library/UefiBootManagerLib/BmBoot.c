...
//6884V112_1+>>
      ...
      for ( ; !(isEndNode(DevicePath)); DevicePath = NEXT_NODE(DevicePath))
      {
        ...
        {
          SATA_DEVICE_PATH                 *SataRemainingDevicePath = (SATA_DEVICE_PATH *) DevicePath;
          //DEBUG((DEBUG_INFO, "SATA device found in BlockIo Device Path\n"));
          if (SataRemainingDevicePath != NULL) {
            if (SataRemainingDevicePath->HBAPortNumber != 0){
              IsOtherSATAPort = TRUE;
              //DEBUG((DEBUG_INFO, "SATA Port %d\n", SataRemainingDevicePath->HBAPortNumber));
            }
          }
        }
        ...
        {
          //DEBUG((DEBUG_INFO, "USB device found in BlockIo Device Path\n"));
        }

        if(DevicePath->Type == 0xAF && DevicePath->SubType == 0xAF)
          break;
      } // End for
