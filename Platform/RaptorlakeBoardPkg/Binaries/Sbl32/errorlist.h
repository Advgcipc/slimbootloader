...
#define CATEGORY_LIST_HELPER1\
    ...
    CATEGORY_DEFINE_HELPER(CATEGORY_ME_COMMUNICATION_FAILURE,        "Failed to communicate with " MAKE_STR(ME_STR) " device or invalid " MAKE_STR(ME_STR) " HW response.") \
    ...
    CATEGORY_DEFINE_HELPER(CATEGORY_CSE_FILE_READ_ERROR,             "CSE File read error.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_CSE_FILE_UPDATE_ERROR,           "CSE File write or update error.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_CSE_FILE_COMPARE_GENERAL_ERROR,  "CSE File compare error.") \
    ...
    CATEGORY_DEFINE_HELPER(CATEGORY_MEMANUF_TEST_RUN_ERROR,          MAKE_STR(ME_STR) "Manuf test failed to run.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_MEMANUF_TEST_RESULT_READ_ERROR,  "Cannot read " MAKE_STR(ME_STR) "Manuf test result.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_MEMANUF_BIST_TEST_FAILED,        MAKE_STR(ME_STR) "Manuf BIST test failed.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_MEMANUF_EOL_TEST_FAILED,         MAKE_STR(ME_STR) "Manuf EOL test failed.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_MEMANUF_OPERATION_FAILED,        MAKE_STR(ME_STR) "Manuf operation failed.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_MEINFO_FEATURE_NOT_FOUND,        "Cannot find " MAKE_STR(ME_STR) "Info feature or feature is not supported.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_MEINFO_FEATURE_COMPARE_ERROR,    MAKE_STR(ME_STR) "Info feature not equals to user value.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_MEINFO_FEATURE_EXECUTION_ERROR,  MAKE_STR(ME_STR) "Info failed to execute specific feature.") \
    ...
    CATEGORY_DEFINE_HELPER(CATEGORY_FWU_FW_ERROR,                    MAKE_STR(FW_STR) " returned an error during the FWU process.") \
    ...
    CATEGORY_DEFINE_HELPER(CATEGORY_GENERAL_ERROR,                   MAKE_STR(FW_STR) " or general environment error.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_ICC_FW_ERROR,                    MAKE_STR(FW_STR) " returned an error on ICC command.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_SOFTSTRAP_INVALID_INPUT_VALUE,   "Invalid update value provided for SoftStrap update.") \
    CATEGORY_DEFINE_HELPER(CATEGORY_SOFTSTRAP_UPDATE_ERROR,          "SoftStrap write or update error.") \
\
...
#define ERROR_LIST_HELPER1 \
    ...
    ERROR_DEFINE_HELPER (SERVICE_EXISTS,                                        CATEGORY_INTERNAL_ERROR,                     "Service already exists.") \
    ERROR_DEFINE_HELPER (ITOUCH_STOPPING_DRIVER_FAILED,                         CATEGORY_INTERNAL_ERROR,                     "Failed to stop iTouch driver.") \
    ERROR_DEFINE_HELPER (FILEIO_ERROR_INVALID_ARGUMENT,                         CATEGORY_IO_ERROR,                           "Invalid file name provided: File name cannot contain any of the following characters: \\*|:?<>") \
    ...
    ERROR_DEFINE_HELPER (HECI_STATUS_LOCATE_DEVICE_ERROR,                       CATEGORY_ME_COMMUNICATION_FAILURE,           "Cannot locate " MAKE_STR(ME_STR) " device.") \
    ...
    ERROR_DEFINE_HELPER (HECI_STATUS_MSG_TRANSMISSION_ERROR,                    CATEGORY_HECI_COMMUNICATION_FAILURE,         "Communication error between application and Intel(R) " MAKE_STR(ME_STR) " module.") \
    ...
    ERROR_DEFINE_HELPER (HECI_STATUS_CANNOT_FOUND_ME_CLIENT,                    CATEGORY_HECI_COMMUNICATION_FAILURE,         "Cannot find " MAKE_STR(ME_STR) " client.") \
    ERROR_DEFINE_HELPER (HECI_STATUS_CANNOT_DISCONNECT,                         CATEGORY_HECI_COMMUNICATION_FAILURE,         "Failure occurred during " MAKE_STR(ME_STR) " disconnect.") \
    ...
    ERROR_DEFINE_HELPER (LPC_DID_ERROR_PCH_NOT_SUPPORTED,                       CATEGORY_UNSUPPORTED_HW,                     MAKE_STR(PCH_STR) " is not supported.") \
    ...
    ERROR_DEFINE_HELPER (CANNOT_ACCESS_PCI,                                     CATEGORY_PCI_ACCESS_ERROR,                   "Could not access PCI device.") \
    ...
    ERROR_DEFINE_HELPER (INVALID_CSE_FILE_NAME,                                 CATEGORY_CSE_FILE_INVALID_INPUT_VALUE,       "The " MAKE_STR(CSE_STR) " File Component requested, File Name is not valid for this operation.") \
    ERROR_DEFINE_HELPER (ERROR_BG_MUST_BE_ENABLED,                              CATEGORY_MEMANUF_BIST_TEST_FAILED,           "Boot Guard must be enabled with Policy 5.") \
    ERROR_DEFINE_HELPER (FPT_INVALID_CONFIG_FORMAT,                             CATEGORY_USER_INPUT_FILE_ERROR,              "Failed to read FPT NVARs config file.") \
    ERROR_DEFINE_HELPER (FW_REGISTER_READ_ERROR,                                CATEGORY_ME_COMMUNICATION_FAILURE,           "Fail to read " MAKE_STR(FW_STR) " Status Register value.") \
    ...
    ERROR_DEFINE_HELPER (ERROR_FW_STATUS_FAILURE,                               CATEGORY_MEMANUF_EOL_TEST_FAILED,            MAKE_STR(FW_STR) " status test failed.") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_OPT_FAILED,                              CATEGORY_MEMANUF_OPERATION_FAILED,           MAKE_STR(ME_STR) "Manuf Operation Failed.") \
    ...
    ERROR_DEFINE_HELPER (NVAR_MCA_WRONG_SERVER_ADDRESS,                         CATEGORY_CSE_FILE_UPDATE_ERROR,              "Invalid server address.") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_BIST_RUNNING_RETRY,                      CATEGORY_MEMANUF_TEST_RUN_ERROR,             "Intel(R) " MAKE_STR(ME_STR) " test is currently running, try again later.") \
    ...
    ERROR_DEFINE_HELPER (ERROR_WLAN_CODE_INVALID,                               CATEGORY_MEMANUF_BIST_TEST_FAILED,           "Current WLAN does not match micro-code, please update WLAN micro-code in " MAKE_STR(FW_STR) ".") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_FOVLOCKCHECK_UNKNOWN,                CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Unable to determine Intel(R) " MAKE_STR(ME_STR) " Manufacturing Mode status.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_FOVLOCKCHECK_NOT_EOM,                CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Intel(R) " MAKE_STR(ME_STR) " is still in Manufacturing Mode.") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_PCH_ID_MISMATCH_ERROR,                         CATEGORY_MEMANUF_BIST_TEST_FAILED,           "Using wrong " MAKE_STR(PCH_STR) " SKU Emulation via Intel (R) FIT vs what's the actual HW Type.") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_FAILED_QUERY_TEST,                       CATEGORY_MEMANUF_BIST_TEST_FAILED,           "Failed to retrieve list of BIST tests to run from " MAKE_STR(FW_STR) ".") \
    ...
    ERROR_DEFINE_HELPER (INVALID_DESC,                                          CATEGORY_SPI_ACCESS_ERROR,                   "Flash descriptor is not valid.") \
    ...
    ERROR_DEFINE_HELPER (HECI_STATUS_DRIVER_ACCESS_DENIED,                      CATEGORY_HECI_COMMUNICATION_FAILURE,         "ME client access denied.") \
    ...
    ERROR_DEFINE_HELPER (FEAT_NOT_RELEVANT,                                     CATEGORY_MEINFO_FEATURE_NOT_FOUND,           "Feature %s is not relevant in the current environment.") \
    ...
    ERROR_DEFINE_HELPER (ERASE_SIZE,                                            CATEGORY_SPI_ACCESS_ERROR,                   "Flash device does not support block erase size of 4KB.") \
    ...
    ERROR_DEFINE_HELPER (SPI_STATUS_FCERR,                                      CATEGORY_SPI_ACCESS_ERROR,                   "FCERR is set. Hardware sequencing failed. Make sure that you have access to target flash area.") \
    ERROR_DEFINE_HELPER (MEMANUF_EOL_CHECKING_VAR_FAILED,                       CATEGORY_CSE_FILE_COMPARE_GENERAL_ERROR,     "Checking variable \"%s\" failed.") \
    ERROR_DEFINE_HELPER (NVAR_MCA_WRONG_VPRO_NIC_TBT_ADDRESS,                   CATEGORY_CSE_FILE_UPDATE_ERROR,              "On board and docked vPro NIC cannot have the same value.") \
    ...
    ERROR_DEFINE_HELPER (NVAR_MCA_WRONG_RETIMER_TBT_ADDRESS,                    CATEGORY_CSE_FILE_UPDATE_ERROR,              "Retimers cannot have the same I2C address.") \
    ...
    ERROR_DEFINE_HELPER (FPROG_VERIFY_GLOBAL_LOCKED_FAILED,                     CATEGORY_CSE_FILE_UPDATE_ERROR,              "End Of Manufacturing Operation failure - Verification failure on " MAKE_STR(ME_STR) " Manufacturing\n" \
                                                                                                                             "Mode Done settings.") \
    ...
    ERROR_DEFINE_HELPER (ME_DISABLED,                                           CATEGORY_INTERNAL_ERROR,                     MAKE_STR(ME_STR) "  disabled.") \
    ...
    ERROR_DEFINE_HELPER (NVAR_MCA_INVALID_PASSWORD,                             CATEGORY_CSE_FILE_UPDATE_ERROR,              "Password does not match the criteria.") \
    ...
    ERROR_DEFINE_HELPER (SETTING_GLOBAL_RESET_FAILED,                           CATEGORY_TOOL_FEATURE_EXECUTION_ERROR,       "Setting Global Reset Failed.") \
    ...
    ERROR_DEFINE_HELPER (FPT_ME_DISABLE_FAILURE,                                CATEGORY_TOOL_FEATURE_EXECUTION_ERROR,       "Unable to detect if the request to disable " MAKE_STR(ME_STR) " succeeded.") \
    ...
    ERROR_DEFINE_HELPER (HECI_STATUS_CANNOT_LOCATE_DRIVER,                      CATEGORY_HECI_COMMUNICATION_FAILURE,         "Cannot locate " MAKE_STR(MEI_STR) " driver.") \
    ERROR_DEFINE_HELPER (FWU_FW_NOT_FOUND,                                      CATEGORY_FWU_FW_ERROR,                       "Unexpected internal " MAKE_STR(FW_STR) " error occurred. Object was not found.") \
    ...
    ERROR_DEFINE_HELPER (ERROR_TOUCH_STATUS_COMPAT_CHECK_FAIL,                  CATEGORY_MEMANUF_BIST_TEST_FAILED,           "Used to indicate compatibility revision check between sensor and " MAKE_STR(ME_STR) " failed,\n" \
    ...
    ERROR_DEFINE_HELPER (CSE_FILE_HASH_ERROR,                                   CATEGORY_INTERNAL_ERROR,                     "Failed to hash " MAKE_STR(CSE_STR) " file data.") \
    ...
    ERROR_DEFINE_HELPER (ME_RECOVERY_MODE,                                      CATEGORY_ME_COMMUNICATION_FAILURE,           "Detected " MAKE_STR(ME_STR) " in recovery mode.") \
    ERROR_DEFINE_HELPER (FPT_DELETE_TOKEN_FAILED,                               CATEGORY_TOOL_FEATURE_EXECUTION_ERROR,       MAKE_STR(FW_STR) " returned status: Erase token failure.") \
    ...
    ERROR_DEFINE_HELPER (FPT_SETDAM_FAILED,                                     CATEGORY_INVALID_USER_INPUT,                 "SetDam command failed.") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_FWUPDATE_OEM_ID,                     CATEGORY_MEMANUF_EOL_TEST_FAILED,            MAKE_STR(FW_STR) " Update OEM ID incorrectly set to 00 or FF.") \
    /* 301-400 */ \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_FWUPDATE_OEM_ID_UNKNOWN,             CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Unable to determine " MAKE_STR(FW_STR) " Update OEM ID status.") \
    ...
    ERROR_DEFINE_HELPER (ERROR_PCH_UNLOCK_STATE,                                CATEGORY_MEMANUF_EOL_TEST_FAILED,            MAKE_STR(PCH_STR) " is unlocked. Disable Delayed Authentication Mode and retry.") \
    ...
    ERROR_DEFINE_HELPER (ERROR_CONFIRM_ARB_SVN,                                 CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Minimum ARB SVN value set on current platform does not match corresponding ARB SVN in " MAKE_STR(FW_STR) " image.") \
    ERROR_DEFINE_HELPER (FWU_FW_INVALID,                                        CATEGORY_FWU_FW_ERROR,                       "Unexpected internal " MAKE_STR(FW_STR) " error occurred. Invalid parameter.") \
    ERROR_DEFINE_HELPER (ERROR_PLATFORM_NAME_FOUND,                             CATEGORY_UNSUPPORTED_HW,                     "Platform name for this " MAKE_STR(PCH_STR) " type not found or not exists.") \
    ...
    ERROR_DEFINE_HELPER (HECI_UNSUPPORTED_MSG_TYPE,                             CATEGORY_HECI_COMMUNICATION_FAILURE,         "Intel (R) " MAKE_STR(ME_STR) " Interface : Unsupported message type.") \
    ...
    ERROR_DEFINE_HELPER (FWU_ALLOWSV_MISSING,                                   CATEGORY_INVALID_CLI_OPTIONS,                "Full " MAKE_STR(FW_STR) " Update using same version is not allowed. Include -allowsv in command line to allow it.") \
    ...
    ERROR_DEFINE_HELPER (FWU_INVALID_PARTID,                                    CATEGORY_INVALID_CLI_OPTIONS,                "Invalid Partition ID. Use a Partition ID which is possible to do Partial " MAKE_STR(FW_STR) " Update on.") \
    ...
    ERROR_DEFINE_HELPER (FWU_FW_DEVICE_ERROR,                                   CATEGORY_FWU_FW_ERROR,                       MAKE_STR(FW_STR) " Flash read/write/erase operation failed.") \
    ...
    ERROR_DEFINE_HELPER (FWU_LOCAL_DIS,                                         CATEGORY_FWU_FW_ERROR,                       MAKE_STR(FW_STR) " Update is disabled. MEBX has options to disable / enable " MAKE_STR(FW_STR) " Update.") \
    ERROR_DEFINE_HELPER (FWU_INVALID_OEM_ID,                                    CATEGORY_FWU_FW_ERROR,                       "Firmware update cannot be initiated because the OEM ID given for " MAKE_STR(FW_STR) " Update did not match the OEM ID in the " MAKE_STR(FW_STR) ".") \
    ERROR_DEFINE_HELPER (FWU_DISPLAY_FW_VERSION,                                CATEGORY_FWU_OPERATION_FAILED,               "Display " MAKE_STR(FW_STR) " Version failed.") \
    ERROR_DEFINE_HELPER (FWU_DOWNGRADE_VETOED,                                  CATEGORY_FWU_FW_ERROR,                       "Update was blocked by one of the " MAKE_STR(FW_STR)" modules.") \
    ...
    ERROR_DEFINE_HELPER (FWU_HOST_RESET_REQUIRED,                               CATEGORY_FWU_FW_ERROR,                       "Host reset is required after the last " MAKE_STR(FW_STR) " Update operation.") \
    ...
    ERROR_DEFINE_HELPER (FWU_RESTORE_POINT_OPERATION_NOT_ALLOWED,               CATEGORY_FWU_FW_ERROR,                       "Get Restore Point Image is not allowed, because " MAKE_STR(FW_STR) " Update is in progress. (The regular FW Update will continue).") \
    ...
    ERROR_DEFINE_HELPER (FWU_RESTORE_POINT_REQUEST_FLASH_IN_RECOVERY,           CATEGORY_FWU_FW_ERROR,                       "Restore Point Image was requested, but it is not allowed because " MAKE_STR(CSE_STR) " is in Recovery Mode.") \
    ERROR_DEFINE_HELPER (FWU_DISPLAY_PART_VERSION,                              CATEGORY_FWU_OPERATION_FAILED,               "Display Partition Version failed.") \
    ERROR_DEFINE_HELPER (FWU_RESTORE_POINT_REQUEST_RESTART_NEEDED,              CATEGORY_FWU_FW_ERROR,                       "Restore Point Image was requested, but there was Full/Partial " MAKE_STR(FW_STR) " Update before without Restart after it.") \
    ...
    ERROR_DEFINE_HELPER (FWU_ERROR_INTEGRITY_FAILED_IUNP,                       CATEGORY_FWU_FW_ERROR,                       "IUNP partition hash and calculated hash are not the same.\n" \
                                                                                                                             "If partition hash is zero - wrong MEU Tool was used to create the partition.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_API_VER_MAJOR_FTPR,                          CATEGORY_FWU_FW_ERROR,                       "Place holder. This error code will not be returned by the " MAKE_STR(FW_STR) ".") \
    ERROR_DEFINE_HELPER (FWU_ERROR_API_VER_MAJOR_NFTP,                          CATEGORY_FWU_FW_ERROR,                       "Place holder. This error code will not be returned by the " MAKE_STR(FW_STR) ".") \
    ERROR_DEFINE_HELPER (FWU_ERROR_API_VER_MAJOR_DLMP,                          CATEGORY_FWU_FW_ERROR,                       "Place holder. This error code will not be returned by the " MAKE_STR(FW_STR) ".") \
    ERROR_DEFINE_HELPER (FWU_ERROR_API_VER_MAJOR_RBEP,                          CATEGORY_FWU_FW_ERROR,                       "Place holder. This error code will not be returned by the " MAKE_STR(FW_STR) ".") \
    ...
    ERROR_DEFINE_HELPER (FWU_ERROR_WRONG_IUP,                                   CATEGORY_FWU_FW_ERROR,                       "Partial " MAKE_STR(FW_STR) " Update: the Update Image contains IUP that is different than the one that was requested to be updated in the Partial Update command.") \
    ...
    ERROR_DEFINE_HELPER (FWU_ERROR_ENABLED_INVALID,                             CATEGORY_FWU_FW_ERROR,                       "Invalid " MAKE_STR(FW_STR) " Update enabled state.") \
    ...
    ERROR_DEFINE_HELPER (FWU_ERROR_WRONG_DATA_OPERATION,                        CATEGORY_FWU_FW_ERROR,                       "FWU_DATA Heci command was sent, but the " MAKE_STR(FW_STR) " Update wasn't started with FWU_START Heci command before it.") \
    ...
    ERROR_DEFINE_HELPER (FWU_INVALID_HECI_CMD,                                  CATEGORY_FWU_FW_ERROR,                       MAKE_STR(FW_STR) " Update process received Heci command message with unknown command type.") \
    ERROR_DEFINE_HELPER (HECI_STATUS_CANNOT_DETECT_ME_STATE,                    CATEGORY_ME_COMMUNICATION_FAILURE,           "Cannot obtain " MAKE_STR(ME_STR) " Mode.") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_FW_VERSIONCHECK,                     CATEGORY_MEMANUF_EOL_TEST_FAILED,            MAKE_STR(ME_STR) " " MAKE_STR(FW_STR) " version mismatch, actual value is - %s.") \
    ...
    ERROR_DEFINE_HELPER (FWU_MAX_IUP_ERROR,                                     CATEGORY_INTERNAL_ERROR,                     "Number of IUPs in " MAKE_STR(FW_STR) " exceeds allowed maximum.") \
    ...
    ERROR_DEFINE_HELPER (FWU_FWSTS_REG,                                         CATEGORY_FWU_FW_ERROR,                       MAKE_STR(FW_STR) " failed to read FWSTS register.") \
    ...
    ERROR_DEFINE_HELPER (FWU_ALLOWSV_RS_MISSING,                                CATEGORY_INVALID_CLI_OPTIONS,                "Full " MAKE_STR(FW_STR) " Update using same version is not allowed. Include /s in command line to allow it.") \
    ERROR_DEFINE_HELPER (ICC_INVALID_MPHY_LENGTH,                               CATEGORY_INVALID_USER_INPUT,                 "Invalid MPHY length.") \
    ERROR_DEFINE_HELPER (FWU_FW_ISH_CFG,                                        CATEGORY_FWU_FW_ERROR,                       MAKE_STR(FW_STR) " failed to set ISH configuration file.") \
    ...
    ERROR_DEFINE_HELPER (NVAR_MCA_PRE_PROV_INVALID_STATE,                       CATEGORY_CSE_FILE_UPDATE_ERROR,              "AMT is provisioned. Updating AMT vars at this stage is not supported.") \
    ...
    ERROR_DEFINE_HELPER (FWU_ERROR_FWSTS_INVALID,                               CATEGORY_FWU_FW_ERROR,                       MAKE_STR(CSE_STR) " is in Recovery Mode but FWSTS registers report Normal Mode.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_BURN_INVALID,                                CATEGORY_FWU_FW_ERROR,                       "The Flash Image that was burned on the platform was corrupted. " MAKE_STR(CSE_STR) " is in Recovery Mode at first boot.") \
    ...
    ERROR_DEFINE_HELPER (RESERVED_627,                                          CATEGORY_FWU_FW_ERROR,                       "") \
    ERROR_DEFINE_HELPER (RESERVED_628,                                          CATEGORY_FWU_FW_ERROR,                       "") \
    ERROR_DEFINE_HELPER (ERROR_NON_INITALIZED_NVAR_FILE,                        CATEGORY_CSE_FILE_COMPARE_GENERAL_ERROR,     "Compare value with non-initialized NVAR file.") \
    ERROR_DEFINE_HELPER (RESERVED_630,                                          CATEGORY_FWU_FW_ERROR,                       "") \
    ERROR_DEFINE_HELPER (FWU_ERROR_FW_TYPE_FTPR,                                CATEGORY_FWU_FW_ERROR,                       "Update FTPR must have the same " MAKE_STR(FW_STR) " Type and Sub-Type as Flash FTPR.") \
    ...
    ERROR_DEFINE_HELPER (FPROG_ERROR_READ_SPI_DATA,                             CATEGORY_SPI_ACCESS_ERROR,                   "An error occurred reading flash data.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_UPID_OEM_ID_EMPTY,                   CATEGORY_MEMANUF_EOL_TEST_FAILED,            "UPID feature is supported while OEM ID FPF was not set.") \
    ...
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_UPID_OEM_PLAT_ID_EMPTY,              CATEGORY_MEMANUF_EOL_TEST_FAILED,            "UPID OEM platform ID is set to zero.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_AMT_HW_NVAR,                         CATEGORY_MEMANUF_EOL_TEST_FAILED,            "[Intel(R) Manageability Hardware Status FPF] is set to disabled, but NVAR [Intel(R) AMT Supported] is enabled.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_AMT_HW_EFS,                          CATEGORY_MEMANUF_EOL_TEST_FAILED,            "[Intel(R) Manageability Hardware Status FPF] is set to disabled, but [Intel(R) Manageability Hardware Status NVAR] is enabled.") \
    ERROR_DEFINE_HELPER (ICC_FW_ERROR_SURVIVABILITY_TABLE_REJECTED_BY_PMC,      CATEGORY_ICC_FW_ERROR,                       "PMC rejected MPHY table.") \
    ERROR_DEFINE_HELPER( ERROR_LAN_I2C_FAILURE,                                 CATEGORY_MEMANUF_BIST_TEST_FAILED,           "LAN I2C protocol failure.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_UNDEFINED,                       CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test is undefined.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_MEM_ERROR,                       CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test memory error.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_DISABLED,                        CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test is disabled.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_GENERIC_FAILURE,                 CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test generic error.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_LOAD_FAILURE,                    CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test failed to load.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_VERIFY_FAILURE,                  CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test verification failure.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_READ_FAILURE,                    CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test reading failure.") \
    ERROR_DEFINE_HELPER (ERROR_IPL_BIST_STATUS_HW_FAILURE,                      CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading test hardware failure.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_UNDEFINED,                       CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test is undefined.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_MEM_ERROR,                       CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test memory error.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_DISABLED,                        CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test is disabled.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_GENERIC_FAILURE,                 CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test generic error.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_LOAD_FAILURE,                    CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test loading failure.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_VERIFY_FAILURE,                  CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test verifying failure.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_READ_FAILURE,                    CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test reading failure.") \
    ERROR_DEFINE_HELPER (ERROR_ISH_BIST_STATUS_HW_FAILURE,                      CATEGORY_MEMANUF_BIST_TEST_FAILED,           "IP Loading ISH test hardware failure.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_UNLOCK,                                      CATEGORY_FWU_FW_ERROR,                       MAKE_STR(CSE_STR) " is in Unlocked Mode. FW Update is not possible.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_SAMF_PPHY_PRESENCE,                  CATEGORY_MEMANUF_EOL_TEST_FAILED,            "SAM and PPHY sub-partitions are not present.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_SAMF_PRESENCE,                       CATEGORY_MEMANUF_EOL_TEST_FAILED,            "SAM sub-partition is not present.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_PPHY_PRESENCE,                       CATEGORY_MEMANUF_EOL_TEST_FAILED,            "PPHY sub-partition is not present.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_ATTR_KB_EMPTY,                       CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Attestation KeyBox NVAR is empty.")\
    ERROR_DEFINE_HELPER (ATTKB_FILE_SIZE_ERROR,                                 CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Attestation KeyBox file size invalid.")\
    ERROR_DEFINE_HELPER (ATTKB_FILE_DATA_INVALID_ERROR,                         CATEGORY_CSE_FILE_READ_ERROR,                "Attestation KeyBox file data is invalid.")\
    ERROR_DEFINE_HELPER (ATTKB_NOT_PROVISIONED,                                 CATEGORY_KEYBOX_INVALID_STATUS,              "Attestation KeyBox is not provisioned.")\
    ERROR_DEFINE_HELPER (NVAR_MCA_WRONG_SEAL_POLICY,                            CATEGORY_CSE_FILE_UPDATE_ERROR,              "Invalid value for Signing Policy.")\
    ERROR_DEFINE_HELPER (NVAR_MCA_WRONG_RESEAL_TIMEOUT,                         CATEGORY_CSE_FILE_UPDATE_ERROR,              "Invalid value for Reseal Timeout.")\
    ERROR_DEFINE_HELPER (READ_PCI_FAILURE,                                      CATEGORY_PCI_ACCESS_ERROR,                   "Read PCI failed.") \
    ERROR_DEFINE_HELPER (WRITE_PCI_FAILURE,                                     CATEGORY_PCI_ACCESS_ERROR,                   "Write PCI failed.") \
    ERROR_DEFINE_HELPER (READ_MMIO_FAILURE,                                     CATEGORY_PCI_ACCESS_ERROR,                   "Read MMIO failed.") \
    ERROR_DEFINE_HELPER (WRITE_MMIO_FAILURE,                                    CATEGORY_PCI_ACCESS_ERROR,                   "Write MMIO failed.") \
    ERROR_DEFINE_HELPER (REBOOT_FAILURE,                                        CATEGORY_PCI_ACCESS_ERROR,                   "Reboot failed.") \
    ERROR_DEFINE_HELPER (SPI_MMIO_FAILURE,                                      CATEGORY_PCI_ACCESS_ERROR,                   "Get SPI MMIO failed.") \
    ERROR_DEFINE_HELPER (EXTRACT_RES_FAILURE,                                   CATEGORY_PCI_ACCESS_ERROR,                   "Extracting driver resource failed.") \
    ERROR_DEFINE_HELPER (TEMP_FOLDER_FAILURE,                                   CATEGORY_PCI_ACCESS_ERROR,                   "Creating temporary folder for driver installation failed.") \
    ERROR_DEFINE_HELPER (DRIVER_PATH_FAILURE,                                   CATEGORY_PCI_ACCESS_ERROR,                   "Creating path for driver installation failed.") \
    ERROR_DEFINE_HELPER (WOW64_FAILURE,                                         CATEGORY_PCI_ACCESS_ERROR,                   "Handling WOW64 redirection for driver installation failed.") \
    ERROR_DEFINE_HELPER (DELETE_DRIVER_FAILURE,                                 CATEGORY_PCI_ACCESS_ERROR,                   "Failed to delete driver files when uninstalling.") \
    ERROR_DEFINE_HELPER (DRIVER_CREATE_FAILURE,                                 CATEGORY_PCI_ACCESS_ERROR,                   "Creating driver failed.") \
    ERROR_DEFINE_HELPER (DRIVER_START_FAILURE,                                  CATEGORY_PCI_ACCESS_ERROR,                   "Starting driver failed.") \
    ERROR_DEFINE_HELPER (DRIVER_STOP_FAILURE,                                   CATEGORY_PCI_ACCESS_ERROR,                   "Stopping driver failed.") \
    ERROR_DEFINE_HELPER (DRIVER_REMOVE_FAILURE,                                 CATEGORY_PCI_ACCESS_ERROR,                   "Removing driver failed.") \
    ERROR_DEFINE_HELPER (DRIVER_OPERATION_FAILURE,                              CATEGORY_PCI_ACCESS_ERROR,                   "Driver operation failed.") \
    ERROR_DEFINE_HELPER (WDF_INSTALLER_FAILURE,                                 CATEGORY_PCI_ACCESS_ERROR,                   "WDF installer failure.") \
    ERROR_DEFINE_HELPER (DRIVER_INSTALL_FAILURE,                                CATEGORY_PCI_ACCESS_ERROR,                   "Driver installation failed.") \
    ERROR_DEFINE_HELPER (DRIVER_UNINSTALL_FAILURE,                              CATEGORY_PCI_ACCESS_ERROR,                   "Uninstalling driver failed.") \
    ERROR_DEFINE_HELPER (RESERVED_FILE_NAME,                                    CATEGORY_IO_ERROR,                           "The given file name is a reserved name by OS.") \
    ERROR_DEFINE_HELPER (SVN_MISMATCH,                                          CATEGORY_TOOL_FEATURE_EXECUTION_ERROR,       "Minimum allowed ARB SVN do not match executing ARB SVN.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_GLITCH_DETECTION_DISABLED,           CATEGORY_MEMANUF_EOL_TEST_FAILED,            "HW Glitch Detection is Disabled.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_GLITCH_DETECTION_ENABLED,            CATEGORY_MEMANUF_EOL_TEST_FAILED,            "HW Glitch Detection is Enabled.") \
    /* 701-800 */ \
    ERROR_DEFINE_HELPER (MANUF_ERROR_EOL_MRC_INIT_STATUS,                       CATEGORY_MEMANUF_EOL_TEST_FAILED,            "MRC initialization failed.") \
    ERROR_DEFINE_HELPER (MANUF_ERROR_EOL_GFX_FW_VERSION,                        CATEGORY_MEMANUF_EOL_TEST_FAILED,             MAKE_STR(FW_STR) " version mismatch.") \
    ERROR_DEFINE_HELPER (MANUF_ERROR_HDA_CONTROLLER_INIT_STATUS,                CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Error in HDA initialization.") \
    ERROR_DEFINE_HELPER (MANUF_ERROR_MASTER_ACCESS_PERMISSIONS_CHECK,           CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Master access permissions don't match Intel recommended values.") \
    ERROR_DEFINE_HELPER (MANUF_ERROR_EOL_OPROM_VERSION,                         CATEGORY_MEMANUF_EOL_TEST_FAILED,            "OPROM version mismatch.") \
    ERROR_DEFINE_HELPER (ERROR_GFX_BIST_STATUS_FID_NOT_REACHED,                 CATEGORY_MEMANUF_BIST_TEST_FAILED,           "GFX FID not reached.") \
    ERROR_DEFINE_HELPER (ERROR_GFX_BIST_STATUS_FATAL_ERROR_OCCURED,             CATEGORY_MEMANUF_BIST_TEST_FAILED,           "GFX Fatal error occurred.") \
    ERROR_DEFINE_HELPER (ERROR_GFX_BIST_STATUS_FSP_FAILURE,                     CATEGORY_MEMANUF_BIST_TEST_FAILED,           "GFX FSP failure occurred.") \
    ERROR_DEFINE_HELPER (ERROR_GFX_BIST_STATUS_CSE_RESET_ON_ERROR_OCCURED,      CATEGORY_MEMANUF_BIST_TEST_FAILED,           "GFX CSE reset on error occured.") \
    ERROR_DEFINE_HELPER (VERIFY_EOM_CONFIG_WRONG_SIZE,                          CATEGORY_CSE_FILE_UPDATE_ERROR,              "EOM config file size is incorrect.") \
    ERROR_DEFINE_HELPER (FWU_GSC_BP_COPY,                                       CATEGORY_FWU_FW_ERROR,                       "Update of Boot Partition Copy failed.") \
    ERROR_DEFINE_HELPER (MANUF_EOL_VAR_WITHOUT_CONFIG,                          CATEGORY_INVALID_CLI_OPTIONS,                "EOL VAR requires config file to be provided.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_INCOMPLETE_DATA,                             CATEGORY_FWU_FW_ERROR,                       "FWUpdate Data is shorter than the update image length.") \
    ERROR_DEFINE_HELPER (MANUF_EOL_EOM_NOT_SET,                                 CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Platform is in Manufacturing Mode.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_UPID_NOT_SUPPORTED,                  CATEGORY_MEMANUF_EOL_TEST_FAILED,            "UPID feature is not supported.") \
    ERROR_DEFINE_HELPER (UPID_MAX_COUNTER_EXCEEDED,                             CATEGORY_TOOL_FEATURE_EXECUTION_ERROR,       "UPID Refurbish counter can be increment only 5 times.") \
    ERROR_DEFINE_HELPER (UPID_INVALID_INPUT_PARAMETER,                          CATEGORY_TOOL_FEATURE_EXECUTION_ERROR,       "invalid parameter.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_EOM_CFG_LOCKCHECK_NOT_EOM,           CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Platform is in Manufacturing Mode though NVAR Configuration State is Locked.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_SYSTEM_IS_EOP,                           CATEGORY_MEMANUF_OPERATION_FAILED,           "Intel(R) " MAKE_STR(ME_STR) " is in End Of Post state.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_FLASHLOCKCHECK_NOT_EOM,              CATEGORY_MEMANUF_EOL_TEST_FAILED,            "Platform is in Manufacturing Mode though Flash Protection Mode is set to Protected.") \
    ERROR_DEFINE_HELPER (FPT_FPF_AMT_ENABLED,                                   CATEGORY_CSE_FILE_UPDATE_ERROR,              "It is not allowed to disable [Intel(R) Manageability Hardware Status FPF] when NVAR [Intel(R) Manageability HW Status] is set to enabled.") \
    ERROR_DEFINE_HELPER (HECI_COMMAND_NOT_SUPPORTED,                            CATEGORY_HECI_COMMUNICATION_FAILURE,         "Intel (R) " MAKE_STR(ME_STR) " Interface : Message type is not supported by " MAKE_STR(FW_STR) ".") \
    ERROR_DEFINE_HELPER (MKHI_COMMAND_GENERAL_ERROR,                            CATEGORY_HECI_COMMUNICATION_FAILURE,         "Intel (R) " MAKE_STR(ME_STR) " Interface : " MAKE_STR(FW_STR) " failed for invalid input or general error.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_SKU_MISMATCH,                                CATEGORY_FWU_FW_ERROR,                       "Update to incompatible FW type: Consumer / Corporate or other.") \
    ERROR_DEFINE_HELPER (MEMANUF_ERROR_EOL_UPID_OEM_PLAT_ID_NOT_SET,            CATEGORY_MEMANUF_EOL_TEST_FAILED,            "UPID OEM Platform ID is not set.") \
	ERROR_DEFINE_HELPER (FWU_ERROR_VER_MAN_FAILED_PSEP,                         CATEGORY_FWU_FW_ERROR,                       "Loader failed to verify manifest signature of PSEP.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_GET_EXT_FAILED_PSEP,                         CATEGORY_FWU_FW_ERROR,                       "Some manifest extension is missing in PSEP. Wrong MEU Tool was used to create the partition.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_INTEGRITY_FAILED_PSEP,                       CATEGORY_FWU_FW_ERROR,                       "PSEP partition hash and calculated hash are not the same.\n" \
                                                                                                                             "If partition hash is zero - wrong MEU Tool was used to create the partition.") \
    ERROR_DEFINE_HELPER (FWU_ERROR_API_VER_MAJOR_PSEP,                          CATEGORY_FWU_FW_ERROR,                       "PSEP must have the same major API version as the version inside the list in FTPR,\n" \
                                                                                                                             "in the Update Image for Full Update, in the Flash Image for Partial Update.") \
    ERROR_DEFINE_HELPER (MULTIPLE_GFX_SETUP_ERROR,                              CATEGORY_INVALID_USER_INPUT,                 "Current GFX setup has more than one device. Use ‘-Device’ command line parameter to define device") \
    ERROR_DEFINE_HELPER (D3_AWAKE_FAILED,                                       CATEGORY_INTERNAL_ERROR,                     "Failed to wake up GfxFW.") \
    ERROR_DEFINE_HELPER (D3_RELEASE_FAILED,                                     CATEGORY_INTERNAL_ERROR,                     "Failed to release GfxFW.") \
    ERROR_DEFINE_HELPER (INVALID_SOFTSTRAP_NAME,                                CATEGORY_SOFTSTRAP_INVALID_INPUT_VALUE,      "The softStrap Component requested, softStrap Name is not valid for this operation.") \
    ERROR_DEFINE_HELPER (INVALID_SOFTSTRAP_VALUE,                               CATEGORY_SOFTSTRAP_INVALID_INPUT_VALUE,      "Invalid value for this softStrap.") \
    ERROR_DEFINE_HELPER (SOFTSTRAP_NOT_UPDATEABLE,                              CATEGORY_SOFTSTRAP_UPDATE_ERROR,             "The \"%s\" softStrap is not updatable.") \
    ERROR_DEFINE_HELPER (SOFTSTRAP_NOT_SUPPORTED_ON_SKU,                        CATEGORY_SOFTSTRAP_UPDATE_ERROR,             "This softStrap is not supported on this SKU.") \
    ERROR_DEFINE_HELPER (SS_DEP_SUPPORT_FAILED_VAR,                             CATEGORY_SOFTSTRAP_UPDATE_ERROR,             "This softStrap is not supported on this platform.") \
\
